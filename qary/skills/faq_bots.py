...
from ..constants import FAQ_DOMAINS, FAQ_MIN_SIMILARITY
import spacy_language_model
...
class Bot:
    ...
    def reply(self, statement):
        ...
        mask = question_similarities >= FAQ_MIN_SIMILARITY
        if sum(mask) >= 1:
            responses.extend(list(zip(question_similarities[mask], self.faq['answers'][mask])))
        else:
            responses = [(
                0.10,
                f"I don't know. The only in my list of FAQs that's remotely similar to yours is \"{self.faq['questions'][idx]}\". ")]
        return responses
