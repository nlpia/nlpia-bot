...
class CLIBot:
    ...
    def __init__(
        ...
        self.repliers = [bot.reply if hasattr(
            bot, 'reply') else bot for bot in self.bots if bot is not None]
    ...
    def add_bot(self, bot_name, **bot_kwargs):
        bot_name = bot_name if bot_name.endswith(
            '_bots') else f'{bot_name}_bots'
        ...
        if bot_module.__class__.__name__ == 'module':
            ...
            for bot_class in (c for n, c in module_vars if n.endswith('Bot') and callable(getattr(c, 'reply', None))):
                log.info(
                    f'Adding a Bot class {bot_class} from module {bot_module}...')
                new_bots.append(bot_class(**bot_kwargs))
                new_modules.append(bot_module)
        else:
            log.warning(
                f'FIXME: add feature to allow import of specific bot classes like "qary.skills.{bot_name}"')
    ...
    def log_reply(self, statement, reply):
        ...
        try:
            ...
            log.error(str(e))
            log.info(
                'Saving history.json contents to history.json.swp before overwriting')
            with open(history_path, 'r') as f:
                data = f.read()
    ...
    def reply(self, statement='', context=''):
        ...
        for replier in self.repliers:
            ...
            try:
                ...
                log.warning(
                    f'Error trying to run {replier.__self__.__class__}.{replier.__name__}("{statement}")')
                log.warning(str(e))
        ...
        if len(replies):
            log.info(
                f'Found {len(replies)} suitable replies, limiting to {self.num_top_replies}...')
...
def run_bot():
    ...
    log.debug(
        f'FINAL PROCESSED ARGS AFTER INSTANTIATING CLIBot:\n{vars(constants.args)}\n')
    return BOT
